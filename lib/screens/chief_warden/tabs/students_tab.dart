...
class _StudentsTabState extends State<StudentsTab> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    super.initState();
    _pendingStream = widget.fs.getPendingRegistrations('All');
  ...
  void export() async {
    ...
    final approvedShortStays = await widget.fs.getApprovedShortStays('All').first;
    ...
    final filtered = allMembers.where((m) {
      ...
      final matchesSearch = m.name.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          (m.roomNumber ?? '').toLowerCase().contains(_searchQuery.toLowerCase());
      ...
      if (_statusFilter == 'On Leave') {
        ...
        matchesFilter = !_isStudentOnLeave(m.uid, approvedLeaves) && !_isStudentOnShortStay(m.uid, approvedShortStays);
      }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(color: kPrimary, fontWeight: FontWeight.bold, fontSize: 24),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(s.email, style: const TextStyle(color: Colors.black54)),
                    ],
                  ),
                ),
            ...
            _detailItem(Icons.badge_outlined, 'Roll Number', s.rollNo ?? 'Not Set'),
            _detailItem(Icons.app_registration_sharp, 'Registration Number', s.registrationNo ?? 'Not Set'),
            _detailItem(Icons.phone_outlined, 'Phone', s.phoneNumber ?? 'Not Set'),
            _detailItem(Icons.school_outlined, 'Programme', s.programme ?? 'Not Set'),
            _detailItem(Icons.meeting_room_outlined, 'Hostel', s.hostel ?? 'Not Set'),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF1E3A8A), // Using student primary color for consistency
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Assign Room Number', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
              const SizedBox(height: 4),
              Text('Student: ${s.name}', style: const TextStyle(fontSize: 13, color: Colors.black45, fontWeight: FontWeight.normal)),
            ],
          ),
          content: TextField(
            controller: ctrl,
  ...
  Widget _buildFilterChip(String label, {bool isHostel = false}) {
    final isSelected = isHostel ? _hostelFilter == label : _statusFilter == label;
    return GestureDetector(
      onTap: () => setState(() => isHostel ? _hostelFilter = label : _statusFilter = label),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
        decoration: BoxDecoration(
          color: isSelected ? kPrimary : Colors.white,
          borderRadius: BorderRadius.circular(25),
          border: Border.all(color: isSelected ? kPrimary : Colors.black.withValues(alpha: 0.08)),
                ...
                children: [
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          prefixIcon: Icon(Icons.search_rounded, color: kPrimary, size: 22),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.circular(16),
                      boxShadow: [BoxShadow(color: kPrimary.withValues(alpha: 0.08), blurRadius: 8, offset: const Offset(0, 3))],
                    ),
                    child: PopupMenuButton<String>(
                      icon: Icon(Icons.filter_list_rounded, color: _hostelFilter == 'All' ? Colors.black54 : kPrimary),
          ...
          child: StreamBuilder<List<VistaUser>>(
            ...
            builder: (context, pendingSnap) {
              ...
              if (_hostelFilter != 'All') {
                pending = pending.where((s) => s.hostel == _hostelFilter).toList();
              }
                    ...
                    builder: (context, leaveSnap) {
                      return StreamBuilder<List<ShortStayRequest>>(
                        ...
                        builder: (context, ssSnap) {
                          if (memberSnap.connectionState == ConnectionState.waiting && pendingSnap.connectionState == ConnectionState.waiting) {
                          ...
                          var filtered = allMembers.where((m) {
                            ...
                            final matchesSearch = m.name.toLowerCase().contains(_searchQuery.toLowerCase()) || (m.roomNumber ?? '').toLowerCase().contains(_searchQuery.toLowerCase());
                            bool matchesFilter = true;
                            if (_statusFilter == 'On Leave') {
                              matchesFilter = _isStudentOnLeave(m.uid, approvedLeaves);
                            } else if (_statusFilter == 'Short Stay') {
                          ...
                          } else if (_statusFilter != 'All' || _hostelFilter != 'All') {
                             String hostelPart = _hostelFilter == 'All' ? '' : ' from $_hostelFilter';
                             switch (_statusFilter) {
                               case 'In Campus':
                                 emptyTitle = 'Quiet Corridor';
                                 emptySubtitle = 'All our students$hostelPart seem to be away or on leave right now.';
                                 emptyIcon = Icons.nightlife_rounded;
                                 break;
                               case 'On Leave':
                                 emptyTitle = 'All Students Present';
                                 emptySubtitle = 'It looks like everyone$hostelPart is currently on campus. No active leaves!';
                                 emptyIcon = Icons.home_rounded;
                                 break;
                               case 'Short Stay':
                                 emptyTitle = 'No Active Short Stays';
                                 emptySubtitle = 'There are no visiting day scholars$hostelPart at the moment.';
                                 emptyIcon = Icons.timer_off_rounded;
                                 break;
                               default:
                                 if (_hostelFilter != 'All') {
                                   emptyTitle = 'No Students in $_hostelFilter';
                                   emptySubtitle = 'There are no students registered in this hostel yet.';
                                 }
                             }
                          }
                          ...
                          return Column(
                            ...
                            children: [
                                      ...
                                      GestureDetector(
                                        ...
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: kPrimary,
                                            borderRadius: BorderRadius.circular(16),
                                            boxShadow: [BoxShadow(color: kPrimary.withValues(alpha: 0.3), blurRadius: 10, offset: const Offset(0, 4))],
                                          ),
                                          child: Row(
                                            children: [
                                              Container(
                                              ...
                                              Expanded(
                                                child: Column(
                                                  crossAxisAlignment: CrossAxisAlignment.start,
                                                  children: [
                                                    Text('${pending.length} Registration Request${pending.length > 1 ? 's' : ''}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 14)),
                                                    Text(_showRequests ? 'Tap to hide detail' : 'Tap to review and approve', style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontSize: 11, fontWeight: FontWeight.w500)),
                                                  ],
                                                ),
                                              ),
                                              Icon(_showRequests ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded, color: Colors.white70),
                                            ],
                                          ),
                                        ),
                                      ),
                                                    ...
                                                    child: Row(
                                                      children: [
                                                        CircleAvatar(
                                                          radius: 20,
                                                          backgroundColor: kPrimary.withValues(alpha: 0.1),
                                                          child: Text(s.name.isNotEmpty ? s.name[0].toUpperCase() : 'S', style: const TextStyle(color: kPrimary, fontWeight: FontWeight.bold)),
                                                        ),
                                                        ...
                                                        Row(
                                                          ...
                                                          children: [
                                                            GestureDetector(
                                                              onTap: () async => widget.fs.denyStudent(s.uid),
                                                              child: Container(
                                                                padding: const EdgeInsets.all(10),
                                                                decoration: BoxDecoration(color: Colors.red.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.red.withValues(alpha: 0.15))),
                                                            ...
                                                            GestureDetector(
                                                              ...
                                                              child: Container(
                                                                padding: const EdgeInsets.all(10),
                                                                decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.green.withValues(alpha: 0.15))),
                              ...
                              if (filtered.isEmpty)
                                Expanded(child: WardenEmptyState(icon: emptyIcon, title: emptyTitle, subtitle: emptySubtitle))
                              else
                                Expanded(
                                          ...
                                          children: [
                                            Stack(
                                              children: [
                                                CircleAvatar(
                                                  radius: 24,
                                                  backgroundColor: kPrimary.withValues(alpha: 0.1),
                                                  child: Text(m.name.isNotEmpty ? m.name[0].toUpperCase() : 'S', style: const TextStyle(color: kPrimary, fontWeight: FontWeight.bold, fontSize: 18)),
                                                ),
                                                Positioned(
                                                  right: 0,
                                                  bottom: 0,
                                            ...
                                            Expanded(
                                              child: Column(
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text(m.name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: Color(0xFF1E293B))),
                                                  Text(m.rollNo != null && m.rollNo!.isNotEmpty ? 'Roll: ${m.rollNo}' : 'Reg: ${m.registrationNo ?? "N/A"}', style: const TextStyle(color: Colors.black45, fontSize: 12)),
                                                ],
                                              ),
                                            ),
                                            Column(
                                              ...
                                              children: [
                                                Text(m.roomNumber ?? 'No Room', style: const TextStyle(fontWeight: FontWeight.w800, color: kPrimary, fontSize: 13)),
                                                Text(m.hostel ?? 'N/A', style: const TextStyle(fontSize: 10, color: Colors.black26, fontWeight: FontWeight.bold)),
                                              ],
                                            ),
                                          ],
                                        ),
